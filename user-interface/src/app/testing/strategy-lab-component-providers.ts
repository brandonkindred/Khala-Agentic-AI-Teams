 ...
 * copy-pasted providers array per describe block/spec file.
 *
 * Preconditions: none.
 ...
 *   then `extraProviders` in order — so an entry in `extraProviders` for a
 *   token already listed (e.g. overriding `StrategyLabPaperTradingService`
 *   with a stub) wins, per Angular's last-provider-for-a-token-wins rule.
 */
