...
@Component({
  ...
  templateUrl: './backend-code-v2-job-status.component.html',
  styleUrl: './backend-code-v2-job-status.component.scss',
})
export class BackendCodeV2JobStatusComponent implements OnInit, OnDestroy {
  @Input() jobId!: string;
  ...
  private poll(): void {
    this.api.getBackendCodeV2Status(this.jobId).subscribe({
      next: (res) => {
        this.status = res;
        this.error = null;
