...
Operator surface in the Agent Console for inspecting an agent's learned
**memory**, reviewing its **rules**, and acting on **rule proposals**
(human-in-the-loop approve / reject). This document is the design contract for
the new `CognitionTabComponent` and its sub-sections; the Figma mockups linked
at the bottom are the visual companion to the wireframes below.
...
graph TD
    ...
    CT --> PR[Proposal Review section]

    AP -.reuses.-> ARS[AgentRegistryApiService<br/>/api/agents]
    MT --> CAS[CognitionApiService]
    RL --> CAS
    PR --> CAS
    CAS -->|HttpClient| API[/api/cognition backend/]

    MT -.shared.-> SH[app-loading-spinner<br/>app-empty-state<br/>app-error-message]
    RL -.shared.-> SH
    PR -.shared.-> SH
```

The panel is a single standalone component with three visually distinct
sections stacked vertically under one shared agent picker. All cognition
...
┌──────────────────────────────────────────────────────────────────────┐
│  ⊙ Cognition                                                          │
│  Inspect an agent's memory, rules, and pending rule proposals.        │
│                                                                        │
...
- On selection change: clear section state and refetch memory + rules +
  proposals for the new `agent_id`.
...
## 6. Proposal Review section (the HITL gate)

The actionable core: pending `RuleProposal`s with **Approve** / **Reject**.
...
BEM naming: `.cognition-tab`, `.cognition-tab__header`, `.memory-event`,
`.rule-row`, `.proposal-card`, with state modifiers (`.is-retired`,
`.is-stale`, `.is-pending`).
...
header, agent picker, memory timeline, rules list, and the two proposal-card
states (a normal `ADD` and a stale-evidence `AMEND` with Approve disabled):
