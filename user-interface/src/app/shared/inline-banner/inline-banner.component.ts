 ...
 *
 * Accessibility invariant: whatever live region the message carries wraps the
 * **message only**. By default the role follows severity (`role="alert"` for
 * error/warning, `role="status"` for success/info), but the `live` input
 * overrides it — `'polite'` forces `role="status"` and `'off'` renders no live
 * region at all. Interactive controls are projected via the default
 * `<ng-content>` into a sibling region outside any live region — some assistive
 * tech does not expose controls nested inside an assertive live region.
 *
...
export class InlineBannerComponent {
   ...
   * what the visual severity would imply. `null` (default) derives the role
   * from `variant`; `'polite'` forces `role="status"` (a warning-colored banner
   * that should not interrupt); `'off'` suppresses the live region entirely
   * (for static, always-present text a screen reader should not re-announce on
   * render).
   */
  readonly live = input<'polite' | 'off' | null>(null);
   ...
   *
   * Preconditions: none.
   * Postconditions: when `live()` is set, maps `'polite'`→`'status'` and
   * `'off'`→`null` (no live region). Otherwise derives from severity: assertive
   * `'alert'` for error/warning (the user must notice a failure now), polite
   * `'status'` for success/info. A `null` result means the template renders no
   * `role` attribute.
   */
