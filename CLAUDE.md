...
backend/
  ...
  agents/
    ...
    agent_platform/registry/ # Agent Console catalog: per-agent YAML manifests → /api/agents
...
- **Agent Console & Registry** — single entry point (UI `/agent-console`) for discovering, inspecting, and running specialist agents. Catalog: [`agent_platform/registry/README.md`](backend/agents/agent_platform/registry/README.md); ephemeral sandbox runner: `agent_provisioning_team/sandbox/README.md`; runs/inputs/diff data layer: [`agent_console/README.md`](backend/agents/agent_console/README.md); conversational single-agent authoring (Agent Studio) and its `AgentDefinition` view-model ↔ `AgentManifest` SoT field mapping: [`agent_team_studio/agent_studio/README.md`](backend/agents/agent_team_studio/agent_studio/README.md).
