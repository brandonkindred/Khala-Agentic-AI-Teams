...
| `CODING_TEAM_MAX_LLM_CALLS` | Per-job hard cap on the total number of LLM calls the coding-team swarm may make in one `run_coding_team_orchestrator` invocation (default `300`, floor `1`, garbage falls back to the default). A budget is bound for the whole job and charged before every model call across the Tech Lead, Senior SWEs, and quality-gate agents (a transport-level retry inside one call charges once — the cap counts logical calls, not wire attempts). On exhaustion the job short-circuits with `status="failed: budget_exhausted"` and makes no further LLM calls. Raise it for genuinely large jobs; lower it to tighten the cost ceiling. |
| `CODING_TEAM_ROUND_MULTIPLIER` | Rounds-per-task multiplier for the swarm's round ceiling, which is `max(CODING_TEAM_MIN_ROUNDS, multiplier × task_count)` instead of a flat constant (default `3`, floor `1`, garbage falls back to the default). Exhausting the ceiling is an explicit `status="failed: max_rounds_exhausted"` — a swarm that merged nothing can never be reported as `completed`. |
| `CODING_TEAM_MIN_ROUNDS` | Floor on the swarm round ceiling regardless of task count (default `10`, floor `1`, garbage falls back to the default). See `CODING_TEAM_ROUND_MULTIPLIER`. |
| `CODING_TEAM_THINKING_MECHANICAL` | When truthy (`1`/`true`/`yes`, case-insensitive) re-enables the platform-default thinking level on the coding team's *mechanical* agents (Tech Lead assignment + the per-task lint and code-review gates); by default those agents run with reasoning disabled (`think=False`) while the implementation and planning agents keep the configured high level. Set this to restore reasoning everywhere. |
