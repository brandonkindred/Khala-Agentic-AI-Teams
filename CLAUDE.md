...
| `STRATEGY_LAB_MAX_UNIVERSE_SYMBOLS` | Issues #525 / #534. Hard ceiling on the asset-class default universe (default `20`, raised from `10` to fit the expanded stocks default that now includes common index/sector ETFs — QQQ, IWM, TLT, EEM, GDX, XLE, XLF). When the cap actually truncates the default a `logger.warning` fires. Under union semantics (#534), `resolve_strategy_symbols` returns the (capped) asset-class default **unioned** with `spec.target_symbols`; `spec.target_symbols` is never truncated. |
