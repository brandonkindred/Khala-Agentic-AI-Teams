...
| `STRATEGY_LAB_MAX_UNIVERSE_SYMBOLS` | Hard ceiling on the asset-class default universe (default `20`). When the cap actually truncates the default a `logger.warning` fires. `resolve_strategy_symbols` returns the (capped) asset-class default **unioned** with `spec.target_symbols`; `spec.target_symbols` is never truncated. |
