...
| `CODING_TEAM_READ_FILE_MAX_BYTES` | Byte ceiling for the Senior SWE agent's read-only `read_file` repo-inspection tool (default `65536`, floored to `1`, blank/garbage falls back to the default). A file larger than the effective limit returns a `file_too_large` error — its contents are never partially returned (no silent truncation). A per-call `max_bytes` argument may request a lower limit but is clamped to this ceiling. The companion `list_files` tool (workspace-sandboxed directory listing, build/VCS/cache dirs skipped) has no cap. Both tools reject absolute paths, `..` traversal, and symlink escapes outside the job workspace. |
