...
_TEMPORAL_PREFIX = "agent_provisioning_team.temporal"
...
def _isolate_temporal_modules():
    """Snapshot and restore the ``agent_provisioning_team.temporal*`` modules.
...
def test_importing_temporal_package_does_not_call_start_team_worker():
    ...
    _purge("agent_provisioning_team.temporal")
    with mock.patch.object(shared.temporal, "start_team_worker") as patched:
        importlib.import_module("agent_provisioning_team.temporal")
        importlib.import_module("agent_provisioning_team.temporal.workflows")
        importlib.import_module("agent_provisioning_team.temporal.start_workflow")
        importlib.import_module("agent_provisioning_team.temporal.sandbox_dispatch")
...
def test_temporal_package_exports_pattern_a_contract():
    ...
    _purge("agent_provisioning_team.temporal")
    from agent_provisioning_team.temporal import (
        ACTIVITIES,
        TASK_QUEUE,
        WORKFLOWS,
...
def test_worker_module_exposes_team_service_entrypoint():
    ...
    from agent_provisioning_team.temporal import worker
...
def test_worker_start_delegates_to_start_team_worker(monkeypatch):
    ...
    import shared.temporal
    from agent_provisioning_team.temporal import ACTIVITIES, TASK_QUEUE, WORKFLOWS
...
def test_registered_in_shared_registry():
    ...
    assert TEAM_TEMPORAL_MODULES["agent_provisioning"] == "agent_provisioning_team.temporal"
