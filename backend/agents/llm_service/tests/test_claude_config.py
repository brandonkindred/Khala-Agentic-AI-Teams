...
def test_resolve_model_falls_back_without_runtime(monkeypatch):
    ...
    assert c.resolve_model(None) == "qwen3.5:9b-mlx"
...
def test_looks_like_claude_model():
    ...
    for m in ("deepseek-v4-pro:cloud", "qwen3.5:9b-mlx", "qwen3-coder:480b-cloud", "", "gpt-4"):
...
def test_resolve_claude_model_ignores_non_claude_per_agent_env(monkeypatch):
    ...
    monkeypatch.setenv("LLM_MODEL_backend", "qwen3.5:9b-mlx")
    assert c.resolve_claude_model("backend") == c.DEFAULT_CLAUDE_MODEL
...
def test_resolve_model_for_provider_dispatches(monkeypatch):
    ...
    monkeypatch.setenv("LLM_MODEL", "qwen3.5:9b-mlx")
    assert c.resolve_provider() == "ollama"
    assert c.resolve_model_for_provider(None) == "qwen3.5:9b-mlx"

    monkeypatch.setenv("LLM_PROVIDER", "claude")
    # qwen3.5:9b-mlx is not a Claude model, so the Claude path falls back to the default.
