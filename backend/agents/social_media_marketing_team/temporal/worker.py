"""Temporal worker for the social media marketing team."""
...
from social_media_marketing_team.temporal.activities import run_team_job_activity
...
def create_social_marketing_worker(client: Optional[object] = None) -> Optional[Worker]:
    ...
    if _activity_executor is None:
        _activity_executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="social-marketing-temporal-activity"
        )
    worker = Worker(
        client,
        task_queue=TASK_QUEUE,
        workflows=[SocialMarketingTeamWorkflow],
        activities=[run_team_job_activity],
        activity_executor=_activity_executor,
        max_concurrent_activities=2,
    )
    logger.info("Social marketing Temporal worker created for task queue %s", TASK_QUEUE)
    return worker
...
async def _run_worker_async() -> None:
    ...
    worker = create_social_marketing_worker(client)
    if worker is None:
        return
    logger.info("Social marketing Temporal worker starting")
    await worker.run()
...
def _worker_thread_target() -> None:
    ...
    try:
        ...
        logger.exception("Social marketing Temporal worker failed: %s", e)
    finally:
...
def start_social_marketing_temporal_worker_thread() -> bool:
    global _worker_thread
    if not is_temporal_enabled():
        return False
    ...
    _worker_thread = threading.Thread(
        ...
        name="social-marketing-temporal-worker",
        daemon=True,
    )
    _worker_thread.start()
    logger.info("Social marketing Temporal worker thread started")
    return True
