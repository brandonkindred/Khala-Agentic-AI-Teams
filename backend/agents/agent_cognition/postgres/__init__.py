...
SCHEMA: TeamSchema = TeamSchema(
    ...
    statements=[
        ...
        # -----------------------------------------------------------------
        # Knowledge-graph ingestion watermarks — one row per agent tracking
        # how far the graph sync worker has consumed this agent's memory into
        # the Graphiti/Neo4j knowledge graph. The watermark is on the event's
        # ``recorded_at`` (arrival time), mirroring prune_events' fold
        # semantics, so a late event (fresh ``recorded_at``) is re-picked-up
        # on the next sync pass. ``last_summary_version`` lets the worker
        # re-ingest a rollup when a recompute advances its version. All
        # advances are monotonic (the worker never regresses a watermark).
        # -----------------------------------------------------------------
        """CREATE TABLE IF NOT EXISTS agent_cognition_graph_watermarks (
            ...
            last_summary_version    INTEGER,
            ingested_count          BIGINT NOT NULL DEFAULT 0,
            updated_at              TIMESTAMPTZ NOT NULL DEFAULT NOW()
        )""",
        # Keyset-pagination index for the sync worker's delta scan: it reads
        # events for one agent ordered by ``(recorded_at, id)`` strictly after
        # the stored watermark, so this composite index keeps that scan from
        # walking the whole partition.
