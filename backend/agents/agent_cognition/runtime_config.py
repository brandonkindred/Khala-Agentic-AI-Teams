...
def read_int_with_floor(name: str, default: int, floor: int) -> int:
    ...
    Postconditions: an unset or non-integer value falls back to ``default``; a
    parsed value is returned as ``max(floor, value)`` (so an operator can lower a
    cadence but not below a safe minimum).
    """
    raw = os.getenv(name)
    if raw is None:
        return default
