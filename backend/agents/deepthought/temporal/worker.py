"""Temporal worker bootstrap for the deepthought team.

Exposes a no-arg ``start_deepthought_temporal_worker_thread`` that the generic
team_service entrypoint invokes at boot via the ``TEAM_TEMPORAL_WORKER_MODULE``
/ ``TEAM_TEMPORAL_WORKER_FUNC`` env vars, so the Temporal worker (and its
connected client) is ready before uvicorn starts accepting requests.

Mirrors ``agent_team_studio.user_agent_founder.temporal.worker.start_user_agent_founder_temporal_worker_thread``
— same contract, same boot hook in docker-compose.
"""

from __future__ import annotations

import logging

from deepthought.temporal import ACTIVITIES, TASK_QUEUE, WORKFLOWS
from shared.temporal import is_temporal_enabled, start_team_worker

logger = logging.getLogger(__name__)


def start_deepthought_temporal_worker_thread() -> bool:
    """Start the deepthought Temporal worker (no-op when disabled).

    Preconditions:
        - Safe to call at import/boot time and more than once.

    Postconditions:
        - Returns True if a worker thread is running (or already running),
          False when Temporal is disabled (``TEMPORAL_ADDRESS`` unset). The
          underlying ``start_team_worker`` is idempotent per team.
    """
    if not is_temporal_enabled():
        return False
    return start_team_worker(
        "deepthought",
        WORKFLOWS,
        ACTIVITIES,
        task_queue=TASK_QUEUE,
    )
