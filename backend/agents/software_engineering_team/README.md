...
- **QA agent** (`qa_agent/agent.py`): per-review result cache, keyed by hashing the whole `QAInput` content plus the resolved model in one shot (same whole-input key shape as code review's submission cache) — but the **chunk-level** caching *policy*: every genuine review outcome is cached regardless of `approved` (never a fallback result), since `QAExpertAgent.run()` is a single atomic call with no reduce phase to short-circuit, unlike the submission cache's approved-only rule. `QA_REVIEW_CACHE_SIZE` (default `256`, `0` disables) controls its capacity.
- **DevOps team specialist agents** (`devops_team/_llm_cache.py`, shared by all eight call sites): one whole-input result cache per single-shot LLM-calling specialist — `InfrastructureAsCodeAgent`, `CICDPipelineAgent`, `DeploymentStrategyAgent` (wired once, through the shared `DevOpsSingleShotAgent.run()`), plus `DevSecOpsReviewAgent`, `DevOpsTaskClarifierAgent`, `InfraDebugAgent`, `InfraPatchAgent`, and `DocumentationRunbookAgent` (each wired at its own call site). Same whole-input key shape and every-genuine-result caching policy as QA's cache; a deterministic early return that never calls the LLM (a task-clarifier gap, an unfixable infra-debug verdict) never consults the cache either way. `ChangeReviewAgent` has no cache of its own — it delegates entirely to `CodeReviewAgent`, already covered by the code-review caches above. Each agent's capacity is its own `DEVOPS_*_CACHE_SIZE` var (default `128`, `0` disables) — see [docs/ENV_VARS.md](../../../docs/ENV_VARS.md).
