...
| **Implementation** | backend_code_v2_team | Backend v2 worker (Java/Python/Node, DBs, APIs, infra-adjacent); phase pipeline: planning → setup → execution → review → problem-solving → documentation → deliver, config-driven via `V2TeamConfig` over the shared phase implementations (not a standalone per-team implementation). Driven by the coding-team engine's Tech Lead |
