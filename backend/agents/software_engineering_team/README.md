...
- **QA agent and every `devops_team` specialist agent** (`shared/llm_response_cache.py`): one shared get/set/key/clear implementation backs `qa_agent.agent.QAExpertAgent.run()`'s per-review result cache and one whole-input result cache per single-shot LLM-calling devops specialist — `InfrastructureAsCodeAgent`, `CICDPipelineAgent`, `DeploymentStrategyAgent` (wired once, through the shared `DevOpsSingleShotAgent.run()`), plus `DevSecOpsReviewAgent`, `DevOpsTaskClarifierAgent`, `InfraDebugAgent`, `InfraPatchAgent`, and `DocumentationRunbookAgent` (each wired at its own call site). Each consumer keys on hashing its whole structured input content plus the resolved model in one shot (same whole-input key shape as code review's submission cache), so any field change naturally busts the key with no explicit invalidation logic; a deterministic early return that never calls the LLM (a task-clarifier gap, an unfixable infra-debug verdict) never consults the cache either way. All nine caches share the **chunk-level** caching *policy*: every genuine outcome is cached regardless of `approved` (never a fallback result), since each is a single atomic call with no reduce phase to short-circuit, unlike the submission cache's approved-only rule. `ChangeReviewAgent` has no cache of its own — it delegates entirely to `CodeReviewAgent`, already covered by the code-review caches above. `QA_REVIEW_CACHE_SIZE` (default `256`, `0` disables) controls QA's capacity; each devops agent's capacity is its own `DEVOPS_*_CACHE_SIZE` var (default `128`, `0` disables) — see [docs/ENV_VARS.md](../../../docs/ENV_VARS.md).
