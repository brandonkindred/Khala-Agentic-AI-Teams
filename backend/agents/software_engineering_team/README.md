...
- **QA agent** (`qa_agent/agent.py`), **Security agent** (`security_agent/agent.py`), and every **`devops_team` specialist agent** that makes its own single-shot LLM call — `InfrastructureAsCodeAgent`, `CICDPipelineAgent`, `DeploymentStrategyAgent` (wired once, through the shared `DevOpsSingleShotAgent.run()`), plus `DevSecOpsReviewAgent`, `DevOpsTaskClarifierAgent`, `InfraDebugAgent`, `InfraPatchAgent`, and `DocumentationRunbookAgent` (each wired at its own call site): each has its own per-review result cache, keyed by hashing the whole input model content plus the resolved model in one shot (same whole-input key shape as code review's submission cache) — but the **chunk-level** caching *policy*: every genuine review outcome is cached regardless of `approved` (never a fallback result), since each agent's `run()` is a single atomic call with no reduce phase to short-circuit, unlike the submission cache's approved-only rule. A deterministic early return that never calls the LLM (a devops task-clarifier gap, an unfixable infra-debug verdict) never consults the cache either way. `ChangeReviewAgent` has no cache of its own — it delegates entirely to `CodeReviewAgent`, already covered by the code-review caches above. The shared plumbing (namespace/build-id suffixing, env-var-driven capacity, key hashing, get/validate/corrupt-entry-delete, fail-open set/clear) lives in one place — `software_engineering_team/shared/review_result_cache.py` — with each agent supplying only its own namespace stem, env var (`QA_REVIEW_CACHE_SIZE` / `SECURITY_REVIEW_CACHE_SIZE`, default `256`; each devops agent's own `DEVOPS_*_CACHE_SIZE`, default `128`; `0` disables), and output model — see [docs/ENV_VARS.md](../../../docs/ENV_VARS.md). Security's model fingerprint is resolved via a Strands model built purely for identity, since that agent calls the LLM through `run_single_shot_review` rather than holding a resolved model itself.
