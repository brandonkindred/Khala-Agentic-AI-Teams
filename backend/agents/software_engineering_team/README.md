...
- **Security agent** (`security_agent/agent.py`): per-review result cache, same whole-input key shape and every-genuine-outcome policy as the QA agent's cache above, keyed on the whole `SecurityInput` plus the resolved model (fingerprinted via a Strands model resolved purely for identity, since this agent calls the LLM through `run_single_shot_review` rather than holding a resolved model itself). `SECURITY_REVIEW_CACHE_SIZE` (default `256`, `0` disables) controls its capacity.
