...
- devops owns standalone infrastructure-only work: creating or changing a CI/CD pipeline definition, provisioning infrastructure as code (e.g. Terraform), and defining deployment/container-orchestration configuration — only when that is the entire task, with no application backend code involved.

When a task needs front-end implementation, route it to target_team "frontend_v2" and include a "frontend_v2" stack. When a task needs backend, platform, server, API, data, or persistence implementation — including a backend task that also touches its own deployment files (e.g. a Dockerfile alongside the service code) — route it to target_team "backend_v2" and include a "backend_v2" stack. When a task is purely infrastructure work as described above, route it to target_team "devops"; no separate stack declaration is needed for devops (it is provisioned automatically from the target_team hint) — only declare "frontend_v2"/"backend_v2" stacks. Do not invent other implementation stacks.
