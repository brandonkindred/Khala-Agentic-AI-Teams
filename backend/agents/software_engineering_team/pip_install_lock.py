...
The lock mechanics themselves are shared with ``unified_api``'s
``_ensure_repo_clone`` (which serializes concurrent clone/fetch of one
per-issue checkout — see ``clone_workspace.clone_lock_path``) via
``shared.concurrency.flock_lock``, rather than each maintaining its own copy
of the same POSIX ``flock`` bookkeeping. Unlike that per-checkout lock, this
one is *not* keyed by repo/worktree path: the resource it protects (the
shared interpreter's ``site-packages``) is the same regardless of which
worktree's ``requirements.txt`` is being installed, so a single fixed path is
correct here — resolved to an absolute path so that same fixed path is what
every worker process actually contends on, regardless of each worker's own
current working directory (``AGENT_CACHE``, like everywhere else it's read
in this codebase, may be relative).
...
def pip_install_lock_path() -> Path:
    ...
    Postconditions:
        - Returns the same path on every call. Pure: no filesystem access
          beyond ``Path.resolve()``'s own symlink-normalization stat calls
          (it neither creates nor opens anything).
        - The returned path is absolute: ``agent_cache_dir()`` may return a
          relative default (``.agent_cache``), and every worker process must
          resolve to the exact same lock file regardless of its own current
          working directory for the cross-process guarantee to hold.
        - The path is not derived from any repo/worktree path — the resource
          ...
          the install.
    """
    return (Path(agent_cache_dir()) / "coding_team" / ".pip_install.lock").resolve()
...
def pip_install_lock() -> Iterator[None]:
    ...
    Postconditions:
        ...
        - The lock is released (and the file closed) before this context
          ...
          block's body raises — see :func:`shared.concurrency.flock_lock`.
        - A failure to open or ``flock`` the lock file (``OSError`` raised
          while *acquiring* :func:`~shared.concurrency.flock_lock.flock_lock`,
          as opposed to one raised by the wrapped block itself) is logged as
          a warning and swallowed: the wrapped block still runs, unguarded.
          Every caller already treats a pip-install failure as non-fatal, so
          a lock-acquisition failure must not newly make the caller fail
          closed. An ``OSError`` raised by the wrapped block itself is not
          caught here and propagates normally — only acquisition failures
          degrade to unguarded.
    """
    lock_path = pip_install_lock_path()
    try:
        lock_path.parent.mkdir(parents=True, exist_ok=True)
    except OSError as e:
        logger.warning("Could not prepare pip install lock directory %s: %s", lock_path.parent, e)
        yield
        return
    with ExitStack() as stack:
        try:
            # Only __enter__ (open + flock) can raise here; the failure this
            # degrades is acquiring the lock, not anything the wrapped block
            # itself does. Using ExitStack (rather than `with flock_lock(...):`
            # directly around `yield`) keeps those two failure modes distinct:
            # a plain `try/except OSError` around `with flock_lock(...): yield`
            # would also catch an OSError raised by the wrapped block itself
            # and incorrectly `yield` a second time.
