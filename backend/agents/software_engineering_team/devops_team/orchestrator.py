...
class DevOpsTeamLeadAgent(BaseTeamLead):
    ...
    def _debug_patch_once(
        ...
        *,
        ...
        Thin wrapper around the standalone ``run_debug_patch_once``; injects
        this team lead's debug/patch agents, execution-tool runner, and
        status-report hook.
        """
        return run_debug_patch_once(
            fix_iter,
            state=state,
            ...
            infra_debug_agent=self.infra_debug_agent,
            infra_patch_agent=self.infra_patch_agent,
            run_execution_tools=self._run_execution_tools,
    ...
    def _run_pipeline(
        ...
        *,
        ...
        def _phase3_branch_write() -> Optional[DevOpsTeamResult]:
            ...
            Thin wrapper around the standalone ``run_phase3_branch_write``;
            converts its typed result into this pipeline's gate contract.
            """
            result = run_phase3_branch_write(
                write_changes=write_changes,
                aggregated_artifacts=aggregated_artifacts,
                repo_path=repo_path,
                task_spec=task_spec,
