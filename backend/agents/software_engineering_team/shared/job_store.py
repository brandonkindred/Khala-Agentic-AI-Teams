...
# Human-in-the-loop pause/answer wrappers — generated by the shared factory so
# this team and coding_team share one contract (see job_service_client.py). The
# ``lambda cd: _client(cd)`` keeps the client factory resolved at call time so
# tests can monkeypatch ``job_store._client`` and the wrappers observe it.
(
