"""Compatibility shim: shared dev-pipeline models moved to ``shared.dev_models``.

The models now live in the neutral top-level package ``shared.dev_models`` so the
...
``shared.dev_models.models`` via ``sys.modules`` so that:
...
New code should import from ``shared.dev_models`` directly.
"""
...
from shared.dev_models import models as _models
