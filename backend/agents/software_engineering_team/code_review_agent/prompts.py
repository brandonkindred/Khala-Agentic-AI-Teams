...
3. When the contract changed, use `find_references` first (falling back to `search_repository` for a plain substring `find_references` does not cover) to find every caller of the enclosing function/method, in this submission and the wider repository; then `read_function`/`read_lines` enough of each caller to decide, in Design-by-Contract terms, whether the NEW code is the defect (it silently broke a contract callers still rely on — the callers are the injured party) or the CALLERS are the defect (they relied on the old, now-superseded contract and must be updated to match the new one — the new code's contract is the correct one going forward). Non-default: `read_file` — reserve it for confirming a whole file/module exists, or when a file is small enough that a full read is simplest, not as the default way to read a caller. State which side you conclude is wrong and why.
