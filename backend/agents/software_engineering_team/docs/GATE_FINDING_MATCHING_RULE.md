...
It specifies, in order: how a raw finding's native fields resolve to a
location and a defect-class token (§1–§2); the match test itself (§3); the
line tolerance and its justification (§4); how ambiguous many-to-many
candidates are resolved deterministically (§5); what the rule deliberately
cannot distinguish (§6); and a worked walkthrough against the two examples
already in `CORPUS_CASE_FORMAT.md` (§7).
...
regex is attempted against that string:
...
digits. A match resolves to `(file_path, line, line)` when the digit group is
present, or to file-known/line-unresolved when it is absent. No match
resolves to fully `unresolved`.
...
function name and a line but no file extension — does not match, and
resolves to `unresolved`, not to a guessed file. A rule that keeps adding
parses to rescue more findings is a rule re-admitting prose judgment through
the back door; one fixed pattern keeps the boundary bright.
...
no other rescue heuristic. See §6.1.
...
checked against `defect_class` by the match test in §3 below — a QA finding
...
2. **Location**:
   ...
   - If `L.line` is not `null`: match requires `F` to be resolved (§2) with
     `F.file_path == L.file_path`, **and** `F`'s resolved line to be a
     numeric line (not file-known/line-unresolved), **and** that line to fall
     within `[L.line - 3, L.line_end + 3]` (tolerance justified in §5). A
     finding that only knows its file, not its line, does **not** satisfy a
...
- 3 lines stays tighter than the typical vertical gap between distinct,
  ...
  §6.4 states the corresponding failure mode directly: dense, unspaced code
  can still bring two distinct defects within tolerance of each other.
...
1. For each candidate pair, compute a distance: for a line-specific label,
   the absolute distance from the finding's resolved line to the nearer end
   of `[L.line, L.line_end]` (0 if the finding's line already falls inside
   the range); for a file-wide label, distance is defined as 0 (there is
   only ever one file-wide "slot" per file per label to fill, so line
   distance does not disambiguate it).
...
paired by reassigning an earlier, lower-priority match — see §6.5. The
...
content — it applies §1–§6 to hypothetical real gate output against those
...
- **Security** emits `category: "injection"`, `location: "user_repo.py:27"`.
  §2.3's regex matches (`user_repo.py` has a `.`-extension, `:27` present),
  resolving to `("user_repo.py", 27, 27)`. Note this resolves to the
  *basename* the free-text string actually contained, not the label's full
  relative path `app/repositories/user_repo.py` — §4's file-path comparison
  is between what each side literally states, and a security finding whose
  free-text location never contains the full relative path fails the file
  match even though a human would recognize the file. §2.3 does not attempt
  to reconcile a basename against a full path; that is a second instance of
  §6.3's limitation (an unresolved-to-full-path finding cannot match), not
  an exception to it. Had the location instead been
  `"app/repositories/user_repo.py:27"`, the match would succeed: gate
  matches, file matches, `27` is within `[24, 30]`, class `injection`
  matches `L2`. **L2 satisfied only when the free-text location includes the
  full relative path.**
- **QA** emits `description: "user_id from the request path reaches the
  ...
  `GATE_FINDING_INVENTORY.md` §2). §2.3's regex matches `user_repo.py` (a
  `.`-extension present) with no digit group, resolving to file-known,
  line-unresolved. Per §4, a label with `line: 27` (not `null`) requires a
  *numeric* resolved line — file-known-only does not satisfy it. **L3 is not
...
means the same file, consistent with §2.4 and §6.3.
