...
def run_pipeline_activity(request: dict[str, Any]) -> dict[str, Any]:
    ...
    from software_engineering_team.coding_team.api.main import (
        RunRequest,
        create_job,
        get_job,
        plan_from_input,
    ...
    if get_engine_provider() is None:
        raise RuntimeError(
            ...
            "software_engineering_team.coding_team.engine_provider.set_engine_provider(...) "
...
# ``software_engineering_team.coding_team.temporal.worker`` (invoked from SE's
# ``_se_startup()`` hook).
