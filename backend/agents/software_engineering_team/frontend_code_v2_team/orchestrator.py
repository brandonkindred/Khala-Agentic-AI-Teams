...
class FrontendDevelopmentAgent(BaseV2DevelopmentAgent):
    ...
    the frontend tooling detection, repo-briefing sets, and the integration-only
    ``run_workflow``, which calls the base class's ``_run_preflight``,
    ``_run_planning_and_branch_setup``, and ``_run_documentation_phase`` for its
    branch-checkout/tooling-verification, planning/feature-branch-creation, and
    documentation steps respectively.
    """
