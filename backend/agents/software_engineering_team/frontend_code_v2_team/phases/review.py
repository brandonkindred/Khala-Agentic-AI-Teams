...
def _review_steps_run_sequentially(llm: LLMClient) -> bool:
    ...
    production caller of ``run_coding_team_orchestrator``, including this team's own
    ``run_microtask_review`` call sites in ``phases/execution.py``) wraps it in a Strands
    ``LLMClientModel`` for reasoning-stream capture
    (``coding_team.reasoning_capture._make_reasoning_llm_getter``), and that wrapper survives
    unchanged through ``worker_factory._v2_text_mode_llm``'s clone path all the way to this module's
    ``llm`` parameter — so a bare ``isinstance(llm, DummyLLMClient)`` misses a dummy client reached
    through that (default, production) path. Unwrap via the model's public ``client`` accessor
