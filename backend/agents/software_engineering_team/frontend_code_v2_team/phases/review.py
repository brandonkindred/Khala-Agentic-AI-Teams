    ...
    *,
    ...
    Preconditions:
        ...
        - ``language`` is forwarded to ``CodeReviewInput`` so the coordinator's
          chunk reviewer prompts against this team's actual detected language
          (this team's ``_detect_language`` may return ``"angular"``/
          ``"react"``/``"typescript"``, not just one fixed value) instead of
          being silently discarded, as it was before this migration since
          ``REVIEW_PROMPT``/``parse_review_template`` had no language
          placeholder. Defaults to this team's ``PROFILE.default_language``
          ("typescript") so an existing caller that does not pass it yet keeps
