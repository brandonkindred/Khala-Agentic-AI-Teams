...
class TestListReviewThreads:
    ...
    def test_degrades_to_empty_on_graphql_error(self) -> None:
        client = _client_with(lambda _r: httpx.Response(200, json={"errors": [{"message": "no"}]}))
        assert client.list_review_threads("o", "r", 7) == []

    def test_degrades_to_empty_on_http_error(self) -> None:
...
def address_env(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **kw: dispatched.append({"args": a, "github": kw.get("github")}),
    )
...
class TestUnresolvedComments:
    def test_excludes_resolved_and_khala_comments(self, address_env) -> None:
        ...
        unresolved, by_comment = ac._unresolved_comments(fake, "o", "r", 7)
...
class TestHandleComment:
    def test_real_issue_dispatches_replies_and_resolves(self, address_env, monkeypatch) -> None:
        ...
        outcome = ac._handle_comment(fake, "job1", req, _comment(2), thread, "feature")

        assert outcome.outcome == "resolved"
        assert len(address_env["dispatched"]) == 1
        # The dispatch targets the PR's own branch so the fix pushes back to the PR.
        assert address_env["dispatched"][0]["github"]["integration_branch"] == "feature"
        assert fake.replies and fake.replies[0][0] == 2
        assert fake.resolved == ["T2"]

    def test_false_positive_replies_resolves_no_dispatch(self, address_env, monkeypatch) -> None:
        ac, fake, req = address_env["ac"], address_env["fake"], address_env["request"]
    ...
    def test_not_an_issue_is_skipped(self, address_env, monkeypatch) -> None:
        ...
        outcome = ac._handle_comment(fake, "job1", req, _comment(2), None, "feature")
...
class TestAddressCommentsRoute:
    ...
    def route_env(self, monkeypatch: pytest.MonkeyPatch, tmp_path):
        ...
        started: list[tuple] = []
        monkeypatch.setattr(
            ac, "_start_address_comments_thread", lambda *a, **kw: started.append(a)
        )
