...
class TestSetupPhase:
    def test_run_setup_on_existing_repo(self, tmp_path):
        ...
        from software_engineering_team.codegen_team.stacks.backend.profile import run_setup
    ...
    def test_setup_logs_when_scaffolding_commit_fails(self, tmp_path, monkeypatch, caplog):
        ...
        """
        from software_engineering_team.codegen_team.stacks.backend import profile as setup_mod
...
class TestToolAgents:
    ...
    def test_auth_agent(self):
        from software_engineering_team.codegen_team.tool_agents.backend.auth import AuthToolAgent
...
# ---------------------------------------------------------------------------
# CodegenDevelopmentAgent tests (5-phase cycle)
# ---------------------------------------------------------------------------
...
class TestCodegenDevelopmentAgent:
    ...
    def test_build_tool_runners(self):
        ...
        mock_llm = MagicMock()
        dev = CodegenDevelopmentAgent(mock_llm, "backend")
        tool_agents = _build_backend_tool_agents(mock_llm)
...
# ---------------------------------------------------------------------------
# CodegenTeamLead (Tech Lead: Setup + delegate) tests
# ---------------------------------------------------------------------------
...
class TestCodegenTeamLead:
    def test_team_lead_runs_setup_then_delegates(self, tmp_path):
        ...
        lead = CodegenTeamLead(mock_llm, "backend")
        task = Task(
            id="t-test",
            type=TaskType.BACKEND,
            assignee="backend-code-v2",
    ...
    def test_team_lead_propagates_development_handoff_fields(self, tmp_path, monkeypatch):
        ...
        class _DevelopmentAgent:
            def __init__(self, _llm, _stack="backend"):
                pass
        ...
        result = orch.CodegenTeamLead(MagicMock(), "backend").run_workflow(
            repo_path=tmp_path,
            task=task,
...
class TestCodegenDevelopmentAgentBranchReuse:
    def test_existing_feature_branch_is_reused_without_recreation(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(orch, "_backend_run_planning", _run_planning)
        monkeypatch.setattr(
            orch, "_backend_run_execution_with_review_gates", _run_execution_with_review_gates
        )
        monkeypatch.setattr(
            doc_phase,
        ...
        result = orch.CodegenDevelopmentAgent(MagicMock(), "backend").run_workflow(
            repo_path=tmp_path,
            task=task,
            merge_to_development=False,
        )
    ...
    def test_team_lead_warns_exactly_once_when_delegation_succeeds(self, tmp_path, monkeypatch):
        ...
        class _DevelopmentAgent:
            ...
            def run_workflow(self, **kwargs):
                ...
                return CodegenWorkflowResult(task_id="api", success=True)
