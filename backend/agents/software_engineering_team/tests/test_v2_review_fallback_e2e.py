...
def _clear_ambient_code_review_env(monkeypatch):
    ...
    - An inherited ``CODE_REVIEW_MAX_BISECT_DEPTH=0`` would disable the
      ...
      relies on to isolate ``bad.py``'s failure from ``good.py`` (both files
      are small enough to share one map chunk); without bisection, the
...
def _FailBadKeepGood():  # noqa: N802 -- preserves original class-style call-site spelling
    """Backend variant: fails ``bad.py``, keeps ``good.py``."""
    return _FailBadKeepGoodBase(ext="py")
