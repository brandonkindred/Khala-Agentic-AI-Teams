"""Unit tests for planning_adapter: adapt Planning handoff to Tech Lead/Architecture inputs."""
...
def test_adapt_planning_result_success_with_handoff() -> None:
    """adapt_planning_result with success=True and handoff_package returns ProductRequirements and project_overview; hierarchy is None."""
    result = {
        "success": True,
        "handoff_package": {
    ...
    out = adapt_planning_result(result, spec_title="My Project")
    assert isinstance(out, PlanningAdapterResult)
...
def test_adapt_planning_result_raises_when_success_false() -> None:
    ...
    with pytest.raises(ValueError):
        adapt_planning_result({"failure_reason": "No success key."}, spec_title="X")


def test_adapt_planning_result_prd_fallback_from_repo_path(tmp_path: Path) -> None:
    ...
    out = adapt_planning_result(result, spec_title="Project", repo_path=str(tmp_path))
