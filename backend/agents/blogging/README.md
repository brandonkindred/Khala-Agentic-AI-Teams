...
- Web search uses Ollama's web_search API and **requires** `OLLAMA_API_KEY` (e.g. from https://ollama.com/settings/keys). Without it, `/full-pipeline`, `/full-pipeline-async`, `/job/{job_id}/resume`, and `/job/{job_id}/restart` all reject the request with 422 before starting any work, and `GET /health` reports `web_search_configured: false`.
