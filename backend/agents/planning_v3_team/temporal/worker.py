"""Temporal worker for the Planning V3 team."""
...
from planning_v3_team.temporal.activities import run_planning_v3_activity
...
def create_planning_v3_worker(client: Optional[object] = None) -> Optional[Worker]:
    ...
    worker = Worker(
        ...
        workflows=[PlanningV3Workflow],
        activities=[run_planning_v3_activity],
        activity_executor=_activity_executor,
        max_concurrent_activities=2,
    )
    logger.info("Planning V3 Temporal worker created for task queue %s", TASK_QUEUE)
    return worker
...
async def _run_worker_async() -> None:
    ...
    worker = create_planning_v3_worker(client)
    if worker is None:
        return
    logger.info("Planning V3 Temporal worker starting")
    await worker.run()
...
def _worker_thread_target() -> None:
    ...
    try:
        ...
        logger.exception("Planning V3 Temporal worker failed: %s", e)
    finally:
...
def start_planning_v3_temporal_worker_thread() -> bool:
    global _worker_thread
    if not is_temporal_enabled():
        return False
    ...
    _worker_thread = threading.Thread(
        ...
        name="planning-v3-temporal-worker",
        daemon=True,
    )
