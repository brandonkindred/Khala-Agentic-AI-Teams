...
ALREADY DONE — before inventing tasks, check whether the plan's work is already finished. Base this ONLY on explicit evidence that the work is done — a "Work already completed" section (e.g. closed/merged sub-issues). When such evidence shows the plan's work is already finished, DO NOT create tasks to redo it: return an EMPTY "tasks" list, set "already_complete": true, and summarize the proof in "completion_evidence" (which already-done items satisfy this plan). An "Existing repository code" section is CONTEXT only — it is current code the plan may well need to change, and its mere presence is NOT proof of completion, so never set already_complete from it alone. Recreating finished work is wrong; but skipping requested changes because related code already exists is equally wrong — when the evidence of completion is not explicit, create the tasks.
