...
- ``console`` — runs / saved-inputs / diff data layer; present
...
(``agent_platform.registry``, ``agent_platform.console``,
``agent_platform.sandbox``, ``agent_platform.studio``). This module
re-exports nothing.

Boundary:
    * In: manifest catalog (``agent_platform.registry``), console data layer
      (``agent_platform.console``), sandbox lifecycle (acquire / warm /
      teardown / reaper) plus its Temporal wiring (``agent_platform.sandbox``),
      and Studio authoring (``agent_platform.studio``).
