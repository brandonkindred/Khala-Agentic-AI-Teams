...
:class:`~agent_platform.studio.store.AgentStudioConversationStore`. Same public interface
(``create`` / ``get`` / ``append_message`` / ``discard`` / ``__len__`` / ``turn``)
so :class:`~agent_platform.studio.service.AgentStudioService` is agnostic to which one it
...
DDL lives in ``agent_platform.studio.postgres`` and is registered from the unified API
...
from agent_team_studio.assistant_kernel import ConversationTurn
