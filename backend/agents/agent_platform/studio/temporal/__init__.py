...
exports empty ``WORKFLOWS`` / ``ACTIVITIES`` lists. The unified-API lifespan
calls :func:`agent_platform.studio.temporal.worker.start_agent_studio_temporal_worker_thread`
only when Temporal is configured (see
:func:`agent_platform.studio.temporal.dispatch.studio_temporal_enabled`); that
entrypoint no-ops when there is nothing to register.
