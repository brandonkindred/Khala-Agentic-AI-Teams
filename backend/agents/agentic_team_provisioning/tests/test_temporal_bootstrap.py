...
def _purge(prefix: str) -> None:
    ...
    deliberately re-import ``agentic_team_provisioning.temporal`` to observe its
...
def test_workflows_and_package_do_not_call_os_getenv_at_import_time():
    ...
    with mock.patch.object(os, "getenv", wraps=os.getenv) as spy:
        ...
        assert spy.call_count == 0, (
            f"agentic_team_provisioning.temporal.workflows called os.getenv "
            f"{spy.call_count} time(s) at import — trips the temporalio workflow sandbox."
        )
        spy.reset_mock()
        importlib.import_module("agentic_team_provisioning.temporal")
        assert spy.call_count == 0, (
            f"agentic_team_provisioning.temporal.__init__ called os.getenv "
            f"{spy.call_count} time(s) at import — trips the temporalio workflow sandbox."
        )
...
def test_worker_module_exposes_team_service_entrypoint():
    ...
    """
    from agentic_team_provisioning.temporal import worker
    ...
    assert callable(fn), (
        ...
        "agentic_team_provisioning.temporal.worker"
    )
...
def test_worker_start_delegates_to_start_team_worker(monkeypatch):
    ...
    from agentic_team_provisioning.temporal import ACTIVITIES, TASK_QUEUE, WORKFLOWS
    from agentic_team_provisioning.temporal import worker as worker_mod
