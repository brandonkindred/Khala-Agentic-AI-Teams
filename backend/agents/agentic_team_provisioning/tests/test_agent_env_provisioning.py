...
def test_schedule_provision_skips_when_disabled(monkeypatch, tmp_path):
    ...
    store = AgenticTeamStore(db_path=str(db_path))
...
def test_try_begin_and_list(monkeypatch, tmp_path):
    monkeypatch.setenv("AGENTIC_TEAM_AGENT_PROVISIONING_ENABLED", "false")
    import agentic_team_provisioning.agent_env_provisioning as mod
