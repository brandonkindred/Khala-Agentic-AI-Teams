...
Runs a guided workshop (foundation -> personality & voice -> visual identity with palette
selection -> wrap-up), producing structured `BrandingMission` fields for the broader
5-phase brand framework handled by the rest of the branding team.
"""
...
Where it fits naturally in Phase 1, also surface strategic depth: brand purpose, a clear brand promise, \
and a positioning line — and push back gently on generic values or weak differentiators (suggest sharper \
alternatives). Stay sequential: do not skip ahead until the client has confirmed the current topic.
...
- When Phase 1 essentials are solid (company name, description, audience, values, differentiators), \
mention that the brand team can begin drafting initial directions and they'll see results in the brand \
preview.
...
- Use empty string "" for scalar fields the user hasn't provided yet. Use arrays for values, differentiators, \
existing_brand_material, and color_inspiration.
...
Provide 2–4 short follow-up prompts the client could tap to keep the conversation moving. Tailor them \
to the current workshop phase and where you are in the conversation. Examples:
- Phase 1 (Foundation): "What 3 values matter most?", "What makes you different from [competitor]?", \
"Who's your ideal buyer?"
- Phase 2 (Personality & voice): "How should the brand sound?", "Pick where you sit on formal vs casual", \
"Any brands you admire for tone?"
- Phase 3 (Visual): "Any color or mood references?", "Love, like, or dislike each palette?", \
"Prefer vibrant or muted overall?"
- Phase 4 (Wrap-up): "Anything to adjust before we hand off?", "Confirm this summary looks right?"
"""
