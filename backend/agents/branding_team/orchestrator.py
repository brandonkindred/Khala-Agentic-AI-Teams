...
def _apply_fragment(
    merged: dict[str, Any], data: dict[str, Any], nest_under: Optional[str], *, prefer_first: bool
) -> None:
    ...
    Postconditions:
        Mutates ``merged`` in place and returns ``None``. When ``nest_under`` is
        set, ``data`` is placed under it — skipped when ``prefer_first`` and that
        key is already present (the first writer wins). When ``nest_under`` is
        ``None``: with ``prefer_first`` each key is filled only if absent
        (``setdefault``, first writer wins); otherwise ``data`` overwrites
        (last writer wins). No other keys are touched.
    """
    if nest_under:
...
def _merge_named_fragments(
    ...
    *,
    ...
    for child_node_id, nest_under in node_merge.items():
        ...
        _apply_fragment(merged, structured.model_dump(), nest_under, prefer_first=prefer_first)
