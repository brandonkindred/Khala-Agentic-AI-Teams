...
class BrandingStore:
    ...
    def create_brand(
        ...
        # Best-effort, non-blocking: link the brand to the default profile.
        # record_association_async dispatches to a background worker and never
        # raises, so a link failure can't break or slow brand creation.
        record_association_async(ArtifactType.BRAND, "branding", brand_id, label=brand.name)
        return brand
