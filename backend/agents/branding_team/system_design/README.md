...
| [`mission_field_dependency_analysis.md`](./mission_field_dependency_analysis.md) | Evidence-based analysis of which `BrandingMission` fields each phase's agent prompts actually reference — the basis for a future per-phase field allowlist in `phase_input_hash`/`_phase_task`. |
