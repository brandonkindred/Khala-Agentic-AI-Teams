...
| `branding_channel_activation` | The 9 Phase 4 specialist factories (including all 6 channel guides built via the shared `_make_channel_guide` helper) — `channel_compositor` is not one of these 9 and uses `branding_compositor` instead | `phase_agent_key(BrandPhase.CHANNEL_ACTIVATION)` — mostly bounded, template-driven channel-guideline generation; a natural candidate for a lighter model. |
