...
def test_run_coro_offloads_when_loop_running() -> None:
    """run_coro runs a coroutine on a worker thread when a loop is already active."""
    import asyncio
    ...
    async def _driver():
        ...
        # Called synchronously inside a running loop, so run_coro must offload
        # to a worker thread instead of calling asyncio.run on the live loop.
        return run_coro(_val())
