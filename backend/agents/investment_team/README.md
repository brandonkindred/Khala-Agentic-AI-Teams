...
- [`strategy_lab_pipeline.md`](./system_design/strategy_lab_pipeline.md) — the per-cycle pipeline as a narrative of stages (ideate → fetch data → backtest → align → analyze → optionally paper-trade), the winner gate, and the skip paths. Note these stage names are the doc's own descriptive labels, not wire values: the orchestrator emits `designing`/`design_review`/`design_repair`/`coding`/`backtesting`/`aligning`/`analyzing`/`complete`, of which a subscriber receives only the four `_PROGRESS_PHASE_MAP` targets (`ideating`/`coding`/`backtesting`/`analyzing`).
