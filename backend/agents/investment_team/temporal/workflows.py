...
def run_backtest_activity(
    ...
    Postconditions:
        ...
        - Otherwise ``_run_backtest_background`` has run and persisted the job
          ...
          rather than ``completed``. If the worker returned ``missing`` (the job
          row was deleted out from under it via ``DELETE /backtests/jobs/{job_id}``),
          returns a status dict reporting ``missing`` — distinct from
          ``cancelled`` since no cancellation actually happened, and there is no
          job row left to retry against. Returns a ``completed`` status dict when
          the worker returned ``completed``. Any other (non-terminal) return
          value is a postcondition violation of ``_run_backtest_background``
          and raises ``ApplicationError`` rather than being silently reported
          as ``completed``.
    """
