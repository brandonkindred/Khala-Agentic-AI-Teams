...
| `last_resume_determination` is computed correctly for a `ReviewCheckpoint`/`SynthesisCheckpoint` but genuinely never consumed -- every attempt re-runs DESIGN+REVIEW(+SYNTHESIS) from scratch regardless of the determination (see `checkpoints.py`'s "cross-attempt amendment was attempted once" note for why) | `tests/test_strategy_lab_cross_attempt_resume.py` |
