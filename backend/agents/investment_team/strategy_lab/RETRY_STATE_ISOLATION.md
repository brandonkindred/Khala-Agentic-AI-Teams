...
| `last_resume_determination` is computed correctly for a `ReviewCheckpoint`/`SynthesisCheckpoint`, and is only ever consumed (to resume the next attempt past DESIGN+REVIEW, never past code synthesis) when it is `PipelineStage.SYNTHESIS` (a `ReviewCheckpoint`) AND the triggering `SpecImplementabilityError.spec_implicated` is `False` -- since no production raise site sets that today, every real re-entry still re-runs DESIGN+REVIEW(+SYNTHESIS) from scratch regardless of the determination (see `checkpoints.py`'s module docstring for the full `spec_implicated` gating contract and why a `SynthesisCheckpoint` boundary was deliberately excluded, and its "cross-attempt amendment was attempted once" note for why an earlier, unconditional version of this was reverted) | `tests/test_strategy_lab_cross_attempt_resume.py`, `tests/test_strategy_lab_refinement_freeze.py` |
| Temporal-mode `StrategyLabCycleWorkflow.run` mirrors the same gated cross-attempt resume contract as thread mode's `run_cycle` above -- `resume_stage_determinations` is computed for every re-entry, but the next attempt's `run_design_attempt_activity` params (`resume_spec`/`resume_rationale`/`resume_design_context`, plus a seeded drift) are only populated when the just-failed attempt's `spec_implicated` is `False` AND the determination is `PipelineStage.SYNTHESIS`; the seeded drift's prefix is never re-merged into the parent commit log on a subsequent resumed failure (no duplication), and the "PREVIOUS SPEC UNIMPLEMENTABLE" directive is only appended for a `spec_implicated=True` failure | `tests/test_strategy_lab_temporal_workflows.py`, `tests/test_strategy_lab_temporal_workflow_replay.py` |
