...
| `last_resume_determination` is computed correctly for a `ReviewCheckpoint`/`SynthesisCheckpoint`, and is only ever consumed to resume the next attempt when the triggering `SpecImplementabilityError.spec_implicated` is `False` -- since no production raise site sets that today, every real re-entry still re-runs DESIGN+REVIEW(+SYNTHESIS) from scratch regardless of the determination (see `checkpoints.py`'s module docstring for the full `spec_implicated` gating contract, and its "cross-attempt amendment was attempted once" note for why an earlier, unconditional version of this was reverted) | `tests/test_strategy_lab_cross_attempt_resume.py`, `tests/test_strategy_lab_refinement_freeze.py` |
