...
def _design_context_from_checkpoint(data: Dict[str, Any]) -> _DesignPersistContext:
    ...
    A ``ReviewCheckpoint.design_context`` payload is the JSON-shaped dict
    ...
    cross-attempt resume path) needs the real dataclass back, with real
    ``SpecCritique`` instances, or record assembly's
    ``design_context.critiques[i].model_dump()`` calls raise ``AttributeError``
    deep inside a resumed attempt.
