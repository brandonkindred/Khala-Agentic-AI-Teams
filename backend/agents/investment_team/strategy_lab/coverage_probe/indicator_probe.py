...
# predicate_resolution.py; imported back here — after every remaining
# definition in this module — for the two this file's own code still
# calls directly: _extract_subconditions/_union_target_symbols (used by
# run_indicator_probe). The subcond/operand/indicator-call construction
# cluster that used to need _flatten_top_terms/_symbol_gate/_NameStrings/
# _iter_entry_path_assigns from here now lives in subcond_builder.py and
# imports those directly from predicate_resolution.py itself. This
# module has no reverse dependency subcond_builder.py needs back, aside
# from _AND_OPS/_CombinatorOps (imported at subcond_builder.py's own
# bottom, mirroring this cross-import's cycle-safety rationale).
# predicate_resolution.py imports _BLOCK_FIELDS back from this module at
# ITS bottom, forming a two-way cycle; keeping both cross-imports as
# each module's last statement is what makes the cycle safe regardless
# of which module is imported first — see predicate_resolution.py's
# module docstring.
