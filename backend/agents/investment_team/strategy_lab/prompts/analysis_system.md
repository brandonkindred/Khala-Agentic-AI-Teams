...
The sizing line tells you the capital DEPLOYED into each position, and that deployed size IS the per-trade capital at risk and the per-trade loss cap, because a position can lose up to ~100% of what it deploys. Capital used to enter one position cannot enter another, so the deployed size is the real "capital in play" figure. The line renders by sizing rule: "risk X% per trade" deploys X% of the account (fixed fraction); "vol-target X%" sizes the deployment dynamically to target X% annualized volatility, so the deployed fraction varies per trade; "$Y per trade" targets a fixed $Y per position, capped by the position limit (so the deployed amount can be lower). In every case it is the deployed amount — never a stop-multiplied figure — that is the per-trade capital at risk.
