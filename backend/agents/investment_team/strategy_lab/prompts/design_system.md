...
- **Sanity-check per-trade aggressiveness (position size × stop_loss.pct).** The *realised* loss when a stop fires is `position size × stop_loss.pct`. A 10% fraction with a 5% stop realises ~0.5% of equity per trade — defensible. A 25% fraction with a 10% stop realises ~2.5% — aggressive but consistent. A 50% fraction with a 20% stop realises ~10% — likely a typo. Use this to gut-check whether the sizing is sane, but do **not** write this realised number into `max_loss_per_trade_pct`.

- **`max_loss_per_trade_pct` = per-trade capital-at-risk *tolerance*, not the realised after-stop loss.** A trade can lose at most the capital it deploys (worst case = a 100% loss of the position; a stop only *reduces* the realised loss, and gaps/halts can defeat it). So `max_loss_per_trade_pct` is an **upper bound on the deployed position** and MUST satisfy `max_position_pct ≤ max_loss_per_trade_pct`. Set it to your true per-trade loss tolerance (e.g. `max_position_pct = 10` → `max_loss_per_trade_pct ≥ 10`). Setting it *below* `max_position_pct` is a contradiction the reviewer rejects as critical. Leave it unset if you do not want to declare a tolerance.
