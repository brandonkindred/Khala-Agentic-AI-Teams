...
1a. Risk-model check: confirm the narrative treats the deployed position size as the per-trade capital at risk and loss cap, and treats stop-loss / trailing stop / take-profit as separate within-position safeguards analyzed as a distinct dimension. Read the "Sizing / risk" line by rule: "risk X% per trade" deploys X% of the account; "$Y per trade" deploys a fixed Y dollars; "vol-target X%" is a target annual volatility, so the deployed amount is dynamic and not shown — do NOT read "vol-target X%" as X% capital at risk. Strike any claim that derives per-trade risk by multiplying the stop into sizing (deployed-fraction times stop), calls such a stop-multiplied figure the "capital at risk" / "capital in play", or blames low/negative returns on "low effective risk". Stating that the deployed size IS the capital at risk — including that a genuinely small deployment is small capital at risk — is correct and must be preserved; only strike the stop-multiplied conflation and the misattribution of returns to it.
