    ...
    def _run_design_attempt(
        ...
        *,
        ...
        - ``resume_code`` additionally lets a caller skip Phase 1b (code
          ...
          ``SynthesisCheckpoint`` exception. ``resume_code is not None``
          requires ``resume_spec is not None`` (there is no boundary that
          skips synthesis but not design+review); it is never required the
          other way -- ``resume_spec`` alone (``resume_code is None``)
          still skips only Phase 1, exactly as before this parameter
          existed.
        ...
        if resume_code is not None:
            ...
            # durably checkpointed) for this exact evidence chain. Skip the
            # compiler/synthesis-agent call entirely -- the checkpoint's
            # code is reused verbatim, only the shared post-code bookkeeping
            # (fee-default normalization, original_spec/original_code
            # snapshot, boundary emission) still needs to run.
