...
| `coding` | Refinement-round start/completion (`_run_synthesis_loop`), an abort on budget exhaustion, and every zero-trade-repair outcome — reaches a subscriber unchanged as the public `coding` step, its own `_PROGRESS_PHASE_MAP` entry (not folded into `ideating`) | `{ sub_phase: "started", refinement_round }`; `{ sub_phase: "completed", refinement_round, checks_passed, checks_total }`; `{ sub_phase: "aborted", reason }`; zero-trade repair: `{ sub_phase: "zero_trade_repair_committed" \| "zero_trade_repair_rejected" \| "zero_trade_repair_skipped", refinement_round, ... }` |
