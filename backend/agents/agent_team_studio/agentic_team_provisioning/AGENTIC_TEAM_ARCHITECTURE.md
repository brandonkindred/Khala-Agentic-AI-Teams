...
**Persona fields** (`role`, `skills`, `capabilities`, `tools`, `expertise`) are **not** stored on the roster. For `role`, `skills`, `tools`, and `expertise`, the linked **`AgentManifest` is the source of truth**: API responses and validation **join at read time** via `resolve_persona` / `enrich_roster_agent`, returning an **`EnrichedRosterAgent`** (thin ref plus flattened persona fields projected from the manifest). To change one of these fields, edit the manifest — roster PUT does not accept persona-field writes. `capabilities` is not part of the `AgentManifest` schema at all, so it has no manifest-derived default — the roster projection and the invoke-time manifest binding both always resolve it to an empty list. It is still a live field on the generated-agent invoke input, though: a request that explicitly supplies `capabilities` has that value folded into the composed system prompt for that invoke only (never persisted back to the manifest or roster) — unless the same request also supplies an explicit `system_prompt`, which replaces the composed prompt wholesale and so is never spliced with persona fields, `capabilities` included.
...
5. **Incomplete / sparse profiles** — agents whose linked manifest is missing skills, tools, and/or expertise: `incomplete_profile` when all three are missing, `sparse_profile` when exactly two are missing. `capabilities` is never part of this check, since (as above) it is never projected from the manifest.
