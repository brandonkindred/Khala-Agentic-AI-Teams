...
- The corrective-prompt template is defined once in `structured.py` and includes: the validation/parse error string, the schema JSON, and the prior failed reply (from `exc.response_preview` for parse errors, `json.dumps(data)` for validation errors). Keep schemas small to avoid prompt bloat.
