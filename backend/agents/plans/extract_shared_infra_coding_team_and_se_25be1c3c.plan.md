...
> | Item | Status | Notes |
> |---|---|---|
> | A2 `shared_command_runner` | ✅ done | command runner + error parsing extracted; SE consumers repointed. |
...
> | A1 shared job-store ops | ✅ done | the byte-identical HITL pause/answer helpers now live in `job_service_client.py` beside `BaseJobStore` (the pre-existing "eliminates duplicated CRUD wrappers" home — one shared module, not two); each function takes the `client` explicitly so each team's `_client()` — and the `patched_job_store` fixture — still intercepts; reconciled the `get_submitted_answers` `None`→`[]` drift; team-specific `create_job`/`update_job`/`list_jobs` stay per team. |
> | C1 CI/coverage | ✅ done | coding_team runs as its own CI job at the 90% gate (suite ~97%, includes the acyclic guard); the new `shared_*` packages join the `shared_backend` fan-out; and a dedicated `test-shared-infra` job gates the extracted packages + the `coding_team_service` composition root at 90% (92.6% measured), with `shared_git/git_utils.py` carrying over its pre-extraction integration-heavy omission (`shared_infra.coveragerc`). |
