"""Resolve the Planning output workspace from user input.

In Planning, ``repo_path`` is an OUTPUT directory — the workflow writes the
...
Invariants:
    - Every value returned by ``resolve_workspace`` is an existing, writable
      directory created under ``AGENT_CACHE/planning`` — never anywhere else.
...
def _base_root() -> Path:
    ...
    Postconditions:
        - Returns ``<AGENT_CACHE or '.agent_cache'>/planning`` as a ``Path``,
          ...
          ``planning``). A relative root is resolved against the process
          working directory.
    """
    cache_dir = os.environ.get("AGENT_CACHE", "").strip() or ".agent_cache"
    return Path(cache_dir) / "planning"
...
def resolve_workspace(
    ...
    """Resolve a writable Planning output directory from request input.
    ...
    Postconditions:
        - Returns an absolute ``str`` path to a directory that exists and is
          writable, always located under ``AGENT_CACHE/planning`` (created
          here if absent).
