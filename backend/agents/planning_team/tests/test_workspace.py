"""Unit tests for the Planning workspace resolver.
...
def test_empty_repo_path_uses_client_slug_and_job_id(cache):
    ...
    p = Path(out)
    assert p.is_dir()
    assert p == (cache / "planning" / "Acme-Corp" / "job-123").resolve()
...
def test_none_repo_path_and_none_client_falls_back_to_session(cache):
    ...
    p = Path(out)
    assert p.is_dir()
    assert p == (cache / "planning" / "session" / "job-9").resolve()
...
def test_git_ssh_url_maps_to_repo_name(cache):
    ...
    p = Path(out)
    assert p.is_dir()
    assert p == (cache / "planning" / "Home-maintenance-tracker" / "j1").resolve()


def test_git_https_url_without_suffix(cache):
    out = resolve_workspace("https://github.com/owner/repo", None, "j2")
    assert Path(out) == (cache / "planning" / "repo" / "j2").resolve()
...
def test_empty_agent_cache_env_falls_back_to_default(tmp_path, monkeypatch):
    ...
    # relative 'planning'; it falls back to the '.agent_cache' default. Run
    ...
    assert out == (tmp_path / ".agent_cache" / "planning" / "Acme" / "jE").resolve()
    assert out.is_dir()
...
def test_symlink_segment_escaping_root_raises_error(cache):
    ...
    # would otherwise escape AGENT_CACHE/planning.
    base = cache / "planning"
...
def test_slug_strips_leading_dots_no_hidden_dir(cache):
    ...
    assert out == (cache / "planning" / "hidden" / "jh").resolve()
...
def test_git_url_with_query_maps_to_clean_repo_name(cache):
    ...
    assert Path(out) == (cache / "planning" / "repo" / "jq").resolve()
