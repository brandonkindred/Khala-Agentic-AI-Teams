...
from planning_team.phases.document_production import run_document_production  # noqa: E402
...
def test_architecture_overview_compacted_not_truncated(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "planning_team.phases.document_production.get_client",
        lambda agent_key=None: object(),  # not actually used; compact_text is patched
    )
    monkeypatch.setattr(
        "planning_team.phases.document_production.compact_text",
...
def test_architecture_overview_bounded_on_compaction_failure(tmp_path, monkeypatch):
    ...
    from planning_team.phases.document_production import ARCHITECTURE_OVERVIEW_MAX_CHARS
    ...
    monkeypatch.setattr("planning_team.phases.document_production.get_client", boom)
...
def test_compact_architecture_overview_uses_injected_llm(monkeypatch):
    ...
    from planning_team.phases import document_production as dp
