...
def test_planning_run_request():
    r = PlanningRunRequest(
    ...
    r2 = PlanningRunRequest(repo_path="/x", initial_brief="b")
    assert r2.spec_content is None


def test_planning_run_request_optional_repo_path():
    r = PlanningRunRequest(initial_brief="Build a tracker")
    assert r.repo_path is None
    assert r.initial_brief == "Build a tracker"


def test_planning_run_request_spec_only_is_valid():
    r = PlanningRunRequest(spec_content="# Full spec")
    assert r.initial_brief is None
    assert r.spec_content == "# Full spec"


def test_planning_run_request_requires_brief_or_spec():
    with pytest.raises(ValidationError):
        PlanningRunRequest(repo_path="/x")
    with pytest.raises(ValidationError):
        PlanningRunRequest(initial_brief="   ", spec_content="")


def test_planning_run_response():
    r = PlanningRunResponse(job_id="j1", status="running")
    assert r.job_id == "j1"
    assert "job_id" in r.model_dump()


def test_planning_status_response():
    r = PlanningStatusResponse(job_id="j1", status="running", progress=50)
    assert r.progress == 50
    assert r.pending_questions == []


def test_planning_result_response():
    r = PlanningResultResponse(job_id="j1", success=True, handoff_package={"a": 1})
...
def test_open_question_and_option():
    ...
    q = OpenQuestion(id="q1", question_text="Which?", options=[opt], source="planning")
