"""Tests for Planning job_store's user-profile association hook."""

from __future__ import annotations

from typing import Any, List

from planning_team.shared import job_store
...
def test_create_job_records_profile_association(monkeypatch):
    ...
    assert calls == [((ArtifactType.PROJECT, "planning", "job_1"), {"label": "my/repo"})]
...
def test_create_job_label_falls_back_to_job_id(monkeypatch):
    ...
    assert calls == [
        ((ArtifactType.PROJECT, "planning", "job_2"), {"label": "job_2"}),
        ((ArtifactType.PROJECT, "planning", "job_3"), {"label": "job_3"}),
    ]
