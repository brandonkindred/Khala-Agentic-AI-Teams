"""Unit tests for the Planning Temporal start-workflow client-readiness wait.
...
def test_start_planning_workflow_dispatches(monkeypatch):
    """start_planning_workflow resolves the client, builds the workflow-start
    ...
    sw.start_planning_workflow("job-1", "/tmp/ws", "Acme", "brief", None, False, False)
...
def test_start_planning_workflow_rejects_blank_preconditions(job_id, repo_path):
