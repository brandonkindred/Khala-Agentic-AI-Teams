...
class PersonalAssistantOrchestrator:
    ...
    def _handle_email(self, request: OrchestratorRequest, intent: Intent) -> AgentAction:
        ...
        if any(
            word in message_lower for word in ["read", "check", "inbox", "new emails", "unread"]
        ):
            from ..email_agent.models import EmailReadRequest

            emails = self.email_agent.read_emails(
                EmailReadRequest(
    ...
    def _handle_calendar(self, request: OrchestratorRequest, intent: Intent) -> AgentAction:
        ...
        if any(word in message_lower for word in ["schedule", "add", "create", "book"]):
            ...
            suggestions = self.calendar_agent.suggest_schedule(
                ScheduleRequest(
                    user_id=request.user_id,
                    title="Meeting",
                    duration_minutes=60,
    ...
    def _handle_tasks(self, request: OrchestratorRequest, intent: Intent) -> AgentAction:
        ...
        if any(word in message_lower for word in ["add", "buy", "get", "need", "remember"]):
            ...
            result = self.task_agent.add_items_from_text(
            ...
            task_list = self.task_agent.create_list(
                CreateListRequest(
                    user_id=request.user_id,
                    name=list_name,
                )
    ...
    def _handle_deals(self, request: OrchestratorRequest, intent: Intent) -> AgentAction:
        ...
        if any(word in message_lower for word in ["find", "search", "look for"]):
            ...
            result = self.deal_finder.search_deals(
                SearchDealsRequest(
                    user_id=request.user_id,
                    query=query,
                )
            ...
            return AgentAction(
                ...
                action="search_deals",
                result={
                    "deals": [d.model_dump() for d in result.deals],
    ...
    def _handle_documentation(self, request: OrchestratorRequest, intent: Intent) -> AgentAction:
        ...
        if any(word in message_lower for word in ["checklist", "check list"]):
            ...
            checklist = self.doc_generator.generate_checklist(
            ...
            template = self.doc_generator.generate_template(
            ...
            sop = self.doc_generator.generate_sop(
                SOPRequest(
                    user_id=request.user_id,
            ...
            doc = self.doc_generator.generate_process_doc(
                GenerateDocRequest(
                    user_id=request.user_id,
                    doc_type="guide",
    ...
    def _handle_profile(self, request: OrchestratorRequest, intent: Intent) -> AgentAction:
        ...
        result = profile_agent.learn_from_text(
    ...
    def _generate_response(
        ...
        actions_text = "\n".join(
            f"- {a.agent}: {a.action} ({'success' if a.success else 'failed'})" for a in actions
        )
