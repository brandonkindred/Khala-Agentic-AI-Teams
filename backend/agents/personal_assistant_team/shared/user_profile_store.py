...
class UserProfileStore:
    ...
    def _deep_merge(self, base: Dict, updates: Dict) -> None:
        ...
        for key, value in updates.items():
            if key in base and isinstance(base[key], dict) and isinstance(value, dict):
                ...
                existing_set = set(base[key]) if all(isinstance(x, str) for x in base[key]) else None
                if existing_set:
                    for item in value:
    ...
    def get_profile_summary(self, user_id: str) -> Optional[str]:
        ...
        if profile.preferences.dietary_restrictions:
            lines.append(f"Dietary restrictions: {', '.join(profile.preferences.dietary_restrictions)}")
