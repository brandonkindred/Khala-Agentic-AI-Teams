...
``TEAM_TEMPORAL_WORKER_FUNC``); the unified API boots it via
``shared_temporal.teams_registry``. This package ``__init__`` only re-exports —
it never touches ``os.getenv`` or starts a worker, so importing it (including the
