...
|---|---|
| `executor` | Core: subprocess primitives (`run_command`, `CommandResult`), frontend framework detection, `run_pytest`, `run_python_syntax_check`, `run_linter`. Depends on nothing else in this package at module load time. |
...
- **Import-safe:** importing the package has no side effects and starts no
  threads. `executor` is stdlib-only at import time; it lazily imports
...
- Tests that stub subprocess behaviour patch the module where the *symbol
  ...
  `@patch("shared.command_runner.executor.subprocess.run")` for `run_command`,
