...
def start_conversation(req: StartConversationRequest) -> ConversationStateResponse:
    ...
    Returns the initial conversation state (greeting + seeded definition). Maps the
    service error contract to HTTP: ``ValueError`` → 400 (e.g. ``refine`` without a
    source), ``LookupError`` → 404 (unknown source agent).
    """
    try:
