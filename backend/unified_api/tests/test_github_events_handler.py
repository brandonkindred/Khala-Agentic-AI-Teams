...
def test_add_comment_reaction_noop_without_comment_id():
    ...
    with patch("software_engineering_team.github_source.client.GitHubClient", return_value=fake_client):
...
def test_add_comment_reaction_swallows_errors():
    with patch(
        "software_engineering_team.github_source.client.GitHubClient", side_effect=RuntimeError("boom")
    ):
