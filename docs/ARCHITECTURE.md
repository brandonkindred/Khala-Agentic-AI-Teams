...
- **Platform** — `backend/agents/agent_platform/` (registry, console, sandbox, Studio). The Agent Studio Runner sandbox (`sandbox/`) provisions its own per-invocation Docker Compose stack — Postgres/Temporal/Prometheus/Grafana. Shared cognitive substrate is `backend/agents/agent_cognition/`. Cross-cutting infra is `backend/shared/` (`shared.postgres`, `shared.temporal`, `shared.agent_invoke`).
