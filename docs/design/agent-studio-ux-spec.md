...
8. Explicit `process_id` column on `user_agent_founder_runs` instead of overloading `repo_path` to carry the chosen process id. Purely a data‑model cleanup — the UX works either way — but doing it with this work avoids cementing the overload. (Tracked as an open decision in §7.)
