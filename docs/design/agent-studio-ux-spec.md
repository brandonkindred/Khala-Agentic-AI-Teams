...
- **Editing roster entries.** Clicking a roster entry opens a small inline edit form for its **role / skills / tools** (the `AgenticTeamAgent` fields the old team dashboard exposed), persisted via a **new roster‑update endpoint (see §5 item 3)** — so the capability the deleted `agentic-team-dashboard` shell provided is preserved, not lost in the cutover. *(There is no existing public roster/team‑update route to reuse: the provisioning API exposes `GET /teams/{id}/agents` (read‑only) but no `PUT/PATCH` for a roster entry; today the roster is only persisted **internally** by the assistant's `save_team_agents`, reachable solely from the chat flow — so this edit path needs the new endpoint below.)* **`generated`** entries are fully editable; **`registry`** entries show their manifest‑projected fields read‑only with an "override for this team" toggle, so edits never mutate the source manifest. Re‑running the process designer can also re‑propose roster changes (the chat flow), but inline editing is the direct path.
...
3. **Registry → roster bridge** — add `source: "generated"|"registry"` + `manifest_id` to `agentic_team_provisioning/models.py:AgenticTeamAgent` (additive, defaulted). New `POST …/teams/{id}/agents/from-registry`, `DELETE …/teams/{id}/agents/{agent_name}`, and **`PUT …/teams/{id}/agents/{agent_name}`** (update a roster entry's `role`/`skills`/`tools` — the Stage‑3 *Editing roster entries* inline form; for `registry` entries it stores the per‑team **override** without mutating the source manifest). All three back onto the existing internal `save_team_agents` persistence (which has no public route today) and reuse the same Owner/Admin authz (below). The `from-registry` projection maps the `AgentManifest` into the roster fields with this explicit mapping (so `roster_validation.py` needs no change): **`manifest.tags → skills`**, **`manifest.tools → tools`**, **`manifest.summary → role`**, plus `manifest_id = manifest.id` and `source = "registry"` (`capabilities` left empty unless the manifest carries an equivalent). This mapping must be agreed before implementation; it's the contract `roster_validation.py` reads.
   **`from-registry` API contract** (so frontend/backend agree without a second round‑trip):
   - **Request body:** `{ manifest_id: string, overrides?: { role?: string, skills?: string[], tools?: string[] } }`. The server fetches the manifest by `manifest_id`, applies the projection above, then applies any caller `overrides` on top (override wins per field) — overrides exist so the "override for this team" toggle (Stage 3) can persist a per‑team tweak without mutating the source manifest. `agent_name` is derived server‑side from the manifest (the roster key), not sent by the client.
   - **Response:** the **created/updated `AgenticTeamAgent`** roster entry (the full record including `agent_name`, `source: "registry"`, `manifest_id`, and the projected `role`/`skills`/`tools`), so the client can render the new row without re‑fetching the roster. `409` if an entry for that `manifest_id` already exists on the team.
